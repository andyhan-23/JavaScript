...
function example() {
  ...
  /* 네, 그럼 다시 한번 설명해보겠습니다.
  우선 example 함수 안에서만 쓸수 있는 x,y 지역 변수가 선언되있습니다.
  그리고 순차적으로 코드를 실행해 첫번째로 조건문 안에 들어가 있는 x값을 출력하기 위해
  x변수를 참조하는 코드의 스코프인 example함수의 지역 스코프에서 x변수가 선언되었는지 검색한다.
  그럼 함수레벨스코프로 이루어진 var 변수는 지역 변수의 값이 10에서 30으로 재할당 되었으므로 30이 출력됨
  그다음 y변수 같은 경우 let변수로 선언을 하였기 때문에 블록레벨 범위 안에서면 지역 변수로써 쓰여질수 있습니다. 그래서 
  조건문 안에 있는 y=40이라는 값이 지역변수로 선언되었기 때문에 y를 출력
  그 다음 x같은 경우 지역변수를 참조해야 하기 때문에 함수 내의 코드 블록을 참조하여 30을 출력함 그러한 이유는
  함수 내의 코드 블록은 함수 레벨 스코프에 중첩된다.
  그 다음 y값을 출력하기 위해 지역 변수를 찾아보았지만 y는 let으로 이루어져있는 블록 범위 만을 지역변수를 인정하기 때문에
  저기서 y값을 참조 하기 위해 찾아야 하는 값은 조건문안에 있는 40 이 아닌 조건문 밖에 있는 지역변수인 20일 할당받습니다.
